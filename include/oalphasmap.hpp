#ifndef OALP_HASH_MAP_HPP_
#define OALP_HASH_MAP_HPP_

#include <vector>
#include <utility>

// Open addressed linear probing (OALP) hash map I wrote a while ago
// Far better at exploiting hardware optimisations than std::unordered_map
// Deletions are performed by 'tombstoning' indices
// Pre-allocated - quite expensive to resize due to OA.. max 50% load factor, dont want O(N) insertion
...
public:
  ...
  public:
    ...
    explicit Iterator(MapType* hashmap, const std::size_t pos) 
